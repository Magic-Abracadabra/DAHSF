...
# 渐得如意智能自动化办公平台——定义属于你的咒语
## 官方安装包唯一下载地址【目前过审的安装包存在问题（缺少2个关键空文件夹“秘籍书房”和“机关实验室”）；第二次提交可能由于文件读写权限问题或者语言字符适配问题微软方审核暂未通过（目前的内测结果表明国内的计算机全部正常）】
<a href="https://apps.microsoft.com/detail/9n1nnmqgnb9r" title="微软应用商店"><img src="https://get.microsoft.com/images/zh-cn%20light.svg" alt="微软应用商店" title="微软应用商店"></a>
## 基本语法
...
## 使用本平台
### 两种基本交互方式（注：合理设计数据库，可以增加交互方式，比如语音交互、手机遥控）
...
2. 开发脚本文件：“秘籍文件”是本平台的脚本文件，可用记事本编辑开发，亦可被本平台直接打开。如若代码中存在非命令语句，平台会整体报错提示。
### 查看帮助
...
3. 秘籍文件：本质上是后缀名为“.秘籍”的文本文件，可以被本平台打开；
4. 数据词：在一个句子中模式或内容灵活多样且内容难以直接被上下文完全预测，但是可以被归类的句子片段。例如，在句子
```plaintext
...
5. 关键词：句子中除去数据词以外的部分，它们稳定而且具备上下文关联性强的特点，构成了句子的框架。例如上述案例中，“进入”是一个关键词，句子的框架可以看成“进入（网址）”。当关键词组成一定的框架后，在框架的空缺处填充恰当类型的数据词，便可以形成正确而完整的句子。
